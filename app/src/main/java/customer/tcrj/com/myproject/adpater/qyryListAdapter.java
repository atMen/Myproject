 ...
 * date: 2017/9/27 .
 */
public class qyryListAdapter extends BaseQuickAdapter<qycx.DataBean, BaseViewHolder>{
    private Context mContext;

    public qyryListAdapter(@Nullable List<qycx.DataBean> data, Context context) {
    ...
    protected void convert(BaseViewHolder helper, qycx.DataBean item) {
        ...
        Log.e("TAG","rdjg:"+rdjg);
        if(rdjg != null){
            helper.setText(R.id.tv_phone, "认定机关："+item.getRDJG());
        }

        String jclb = item.getJCLB();
        Log.e("TAG","rdjg:"+jclb);
        if(jclb != null){
            helper.setText(R.id.tv_phone, "奖罚类别："+item.getRDJG());
        }
