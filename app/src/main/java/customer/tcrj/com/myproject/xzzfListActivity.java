...
import customer.tcrj.com.myproject.adpater.xzzfListAdapter;
...
public class xzzfListActivity extends BaseActivity implements BaseQuickAdapter.OnItemClickListener {
    ...
    private xzzfListAdapter detailAdapter;
    private List<xzzfInfo.DataBean> beanList;
    ...
    protected void setView() {
        ...
        title_tv.setText("行政执法信息");
        layout_search_result.setOnClickListener(this);
        btnback.setOnClickListener(this);
        edtsearch.setHint("请输入受检单位名称 / 案件编号");
        ...
        mRecyclerView.setAdapter(detailAdapter = new xzzfListAdapter(beanList, this));
...
//        }
    ...
    private void getData(final int num,String qyname,String xycode,String zzjgcode) {
        ...
        Log.e("TAG","key："+key);
        mMyOkhttp.post()
                .url(ApiConstants.xzzflistApi+key)
//                .addParam("AJBH",qyname == null? "":qyname)
//                .addParam("SJDWMC",xycode == null? "":xycode)
//                .addParam("ZT",zzjgcode)
                ...
                .tag(this)
                .enqueue(new GsonResponseHandler<xzzfInfo>() {
                    @Override
...
//                        Toast.makeText(qyListActivity.this, error_msg, Toast.LENGTH_SHORT).show();
                    ...
                    @Override
                    public void onSuccess(int statusCode, xzzfInfo response) {
//                        Toast.makeText(qyListActivity.this, response.toString(), Toast.LENGTH_SHORT).show();
                        ...
                        if(response.getStat() == null){
                            ...
                            if(response.getStat().equals("101")){
                                Utils.toLogin(xzzfListActivity.this);
                            }
                        }
    ...
    //上拉加载更多数据
    private void loadMoreData(xzzfInfo response,boolean isError) {

        if (response == null) {
            if(isError){

                detailAdapter.loadMoreFail();
                Toast.makeText(xzzfListActivity.this, getResources().getString(R.string.data_failed), Toast.LENGTH_SHORT).show();
            }else{
                detailAdapter.loadMoreFail();
            }
            ...
            if(pageNum > response.getTotalPageCount()){//没有更多数据
                detailAdapter.loadMoreFail();
                Toast.makeText(xzzfListActivity.this, getResources().getString(R.string.data_nomore), Toast.LENGTH_SHORT).show();
            }else{
                pageNum++;
    ...
    //下拉刷新
    private void loadData(List<xzzfInfo.DataBean> response,boolean isError) {
...
//                Toast.makeText(xzzfListActivity.this, getResources().getString(R.string.data_failed), Toast.LENGTH_SHORT).show();
    ...
    List<xzzfInfo.DataBean> onclicresponse;
    @Override
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {

        if(onclicresponse != null){
            xzzfInfo.DataBean dataBean1 = onclicresponse.get(position);
            Bundle bundle = new Bundle();
            bundle.putSerializable("xzzfinfo",dataBean1);
            toClass(this,xzzfListInfoActivity.class,bundle);
        }
